...
template <typename T> class IObserverHandle {
public:
  ~IObserverHandle() {}
  virtual ERROR_CODE_T notifyObserver(T eventInfo) = 0;
};

template <> class IObserverHandle<void> {
public:
  ~IObserverHandle() {}
...
template <class C>
class VoidObserverHandleWithObject : public IObserverHandle<void> {
public:
  VoidObserverHandleWithObject(C *targetObject, ERROR_CODE_T (C::*notify)(void))
      : m_pTarget(targetObject), m_notify(notify) {}
  VoidObserverHandleWithObject(shared_ptr<C> targetObject,
                               ERROR_CODE_T (C::*notify)(void))
      : m_target(targetObject), m_notify(notify), m_pTarget(NULL) {}
...
class ObserverHandleWithObject : public IObserverHandle<T> {
public:
  ObserverHandleWithObject(C *targetObject, ERROR_CODE_T (C::*notify)(T))
      : m_pTarget(targetObject), m_notify(notify) {}
  ObserverHandleWithObject(shared_ptr<C> targetObject,
                           ERROR_CODE_T (C::*notify)(T))
      : m_target(targetObject), m_notify(notify), m_pTarget(NULL) {}
...
public:
  ...
  virtual ERROR_CODE_T notifyObserver(void) { return m_notify(); }
...
public:
  ...
  template <class C>
  shared_ptr<IObserverHandle<T>>
  registerObserver(C *target, ERROR_CODE_T (C::*notify)(T)) {
    CSimpleLock myLock(&m_cs);
    shared_ptr<IObserverHandle<T>> shared =
        make_shared<ObserverHandleWithObject<C, T>>(target, notify);
  ...
  template <class C>
  shared_ptr<IObserverHandle<T>>
  registerObserver(shared_ptr<C> target, ERROR_CODE_T (C::*notify)(T)) {
    CSimpleLock myLock(&m_cs);
    shared_ptr<IObserverHandle<T>> shared =
        make_shared<ObserverHandleWithObject<C, T>>(target, notify);
    observers.push_back(shared);
  ...
  shared_ptr<IObserverHandle<T>> registerObserver(ERROR_CODE_T (*notify)(T)) {
    CSimpleLock myLock(&m_cs);
    shared_ptr<IObserverHandle<T>> shared =
        make_shared<ObserverHandle<T>>(notify);
    observers.push_back(shared);
    return shared;
  }
...
public:
  ...
  template <class C>
  shared_ptr<IObserverHandle<void>>
  registerObserver(C *target, ERROR_CODE_T (C::*notify)(void)) {
    CSimpleLock myLock(&m_cs);
    shared_ptr<IObserverHandle<void>> shared =
        make_shared<VoidObserverHandleWithObject<C>>(target, notify);
    observers.push_back(shared);
    return shared;
  }
  ...
  template <class C>
  shared_ptr<IObserverHandle<void>>
  registerObserver(shared_ptr<C> target, ERROR_CODE_T (C::*notify)(void)) {
    CSimpleLock myLock(&m_cs);
    shared_ptr<IObserverHandle<void>> shared =
        make_shared<VoidObserverHandleWithObject<C>>(target, notify);
    observers.push_back(shared);
  ...
  shared_ptr<IObserverHandle<void>>
  registerObserver(ERROR_CODE_T (*notify)(void)) {
    CSimpleLock myLock(&m_cs);
    shared_ptr<IObserverHandle<void>> shared =
        make_shared<VoidObserverHandle>(notify);
    observers.push_back(shared);
    return shared;
  }
